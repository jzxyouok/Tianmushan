/**************************************************************************
 *
 *  Created by shushaoyong on 2016/10/27.
 *    Copyright © 2016年 踏潮. All rights reserved.
 *
 * 项目名称：浙江踏潮-天目山-h5模版制作软件
 * 版权说明：本软件属浙江踏潮网络科技有限公司所有，在未获得浙江踏潮网络科技有限公司正式授权
 *           情况下，任何企业和个人，不能获取、阅读、安装、传播本软件涉及的任何受知
 *           识产权保护的内容。
 ***************************************************************************/

#import <UIKit/UIKit.h>
#import "PhotoItem.h"

@interface TMSTemplateTool : NSObject

+ (instancetype) sharedInstance;

/**
 *  模版图片上传 文本流方式传输
 *
 *  @param url     <#url description#>
 *  @param params  <#params description#>
 *  @param files   <#files description#>
 *  @param success <#success description#>
 *  @param failure <#failure description#>
 */
- (void) uploadTemplateImagesToUrl:(NSString *)url uploadName:(NSString *)uploadName withFileName:(NSString *)filePath withCompletionBlockWithProgress:(void (^)(NSProgress *pregress))upProgress Success:(void (^)(NSDictionary * responseObject))success withFailure:(void (^)(NSString *error))failure;



/**
 *  模版图片上传 文本流方式传输 拼接传输
 *
 *  @param url     <#url description#>
 *  @param params  <#params description#>
 *  @param files   <#files description#>
 *  @param success <#success description#>
 *  @param failure <#failure description#>
 */
- (void)uploadTemplateImagesToUrl:(NSString *)url uploadName:(NSString *)uploadName withItem:(PhotoItem *)item withCompletionBlockWithProgress:(void (^)(NSProgress *pregress))upProgress Success:(void (^)(NSDictionary * responseObject))success withFailure:(void (^)(NSString *error))failure;


@end
